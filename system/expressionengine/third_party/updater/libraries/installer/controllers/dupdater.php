...
*/
    ...
    public function copy_files_prepare()
    {
        ...
        /*
        // Since EE 2.9.0
        $ellislab_map = directory_map($temp_dir.'system/Ellislab/', 2);
        ...
        */
        ...
        foreach ($this->native_theme_dirs as $val) {
            $native_themes_found[$val] = file_exists($this->maps['themes'].$val);
        }
        ...
        foreach ($themes_map as $dir => $arr) {
            ...
            if (isset($native_themes_found[$dir]) === true) {
                if ($native_themes_found[$dir] === false) continue;
            }
        ...
        /*
        // Loop over all directories in the Ellislab system dir
        foreach ($ellislab_map as $dir => $arr) {
            if (is_array($arr) === false) continue;

            // We don't need that Tests dir
            if (strtolower($dir) == 'tests') continue;

            foreach ($arr as $subdir) {
                $out['dirs'][] = 'system/Ellislab/'.$dir.'/'.$subdir;
            }
        }
        */

        $out['dirs'][] = 'system/EllisLab';
        $out['dirs'][] = 'system/codeigniter';



        // Loop over all directories in the ExpressionEngine system dir
    ...
    public function copy_files()
    {
        ...
        if (strpos($dir, 'system/codeigniter') === 0) {
            ...
            $location = 'system';
        } elseif (strpos($dir, 'system/EllisLab') === 0) {
            $source_dir = $temp_dir.$dir;
            //$basedir = 'codeigniter/system/';
            $location = 'system';
        }
    ...
    public function update_ee()
    {
        ...
        // EE 2.9+
        if (file_exists($installer_dir.'libraries/Update_notices.php')) {
